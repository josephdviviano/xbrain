...
if opts.calc_avg == 1
    ...
    if ~isempty(funcnames2)
        crop_opts2.crop_beginning = opts.crop_beginning2;
        crop_opts2.crop_end = opts.crop_end2;
...
if opts.calc_corr == 1
    ...
    if isempty(funcnames2)
        corrdata_name = calc_within_group_corr(funcnames1,opts,scutoff);  % calculate within-group correlation for funcnames1
    else
        corrdata_name = calc_between_group_corr(funcnames1,funcnames2,opts,scutoff);  % calculate btwn-group correlation for funcnames1 vs. funcnames2
    end
...
for sid = 1:length(funcnames)
    ...
    if ~exist(funcnames{sid})
        ...
        save([funcnames{sid}(1:end-3) 'mat'],'data','datasize','keptvox');
        toc
    end
end
end
...
for sid = 1:length(funcnames)
    ...
    save(fullfile(fpath,[outputname '_subj_' num2str(sid) '_others']),'avg_others','all_keptvox');
